...
import com.sixtyninefourtwenty.bcuddatagenerator.databinding.FragmentTfMaterialDataBinding;
...
public final class TFMaterialDataFragment extends Fragment {

    private FragmentTfMaterialDataBinding binding;

    private final UnitTFMaterialDataAdapter adapter = new UnitTFMaterialDataAdapter(
            (data, a) -> new MaterialAlertDialogBuilder(requireContext())
                    ...
                    .show(),
            (data, a) -> NavHostFragment.findNavController(this).navigate(TFMaterialDataFragmentDirections.showEditUnitTfMaterialDataDialog(a.getCurrentList().stream().mapToInt(UnitTFMaterialData::getUnitId).toArray(), data))
    );
        ...
        public void handleOnBackPressed() {
            if (!adapter.getCurrentList().isEmpty()) {
                new MaterialAlertDialogBuilder(requireContext())
                        ...
                        .setMessage(getString(R.string.items_present_exit_notice, getString(R.string.materials_lowercase)))
                        .setPositiveButton(R.string.exit, (dialog, which) -> NavHostFragment.findNavController(TFMaterialDataFragment.this).popBackStack())
                        .setNegativeButton(android.R.string.cancel, null)
                        .show();
            } else {
                NavHostFragment.findNavController(TFMaterialDataFragment.this).popBackStack();
            }
        }
    };
    ...
    private final ActivityResultLauncher<String> importJson = Utils.createImportTextLauncher(this, jsonString -> {
        try {
            new ListImportConflictResolver<>(adapter.getCurrentList(),
                    UnitTFMaterialData.SERIALIZER.listFromJson(new JSONArray(jsonString)),
    ...
    private final ActivityResultLauncher<String> exportJson = Utils.createExportTextLauncher(this,
            () -> UnitTFMaterialData.SERIALIZER.listToJson(adapter.getCurrentList()).toString());
    ...
    private final MenuProvider provider = new MenuProvider() {
        ...
        public boolean onMenuItemSelected(@NonNull MenuItem menuItem) {
            ...
            if (id == R.id.impor) {
                ...
                exportJson.launch("unit_tf_material_data");
                return true;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        binding = FragmentTfMaterialDataBinding.inflate(inflater, container, false);
    ...
    public void onViewCreated(@NonNull View view, Bundle savedInstanceState) {
        ...
        binding.add.setOnClickListener(v -> NavHostFragment.findNavController(this).navigate(TFMaterialDataFragmentDirections.showAddUnitTfMaterialDataDialog(adapter.getCurrentList().stream().mapToInt(UnitTFMaterialData::getUnitId).toArray())));
        binding.dataList.setLayoutManager(new LinearLayoutManager(requireContext()));
        binding.dataList.setAdapter(adapter);

        AddUnitTFMaterialDataDialog.registerDataCallback(this, newData -> ListAdapters.addElement(adapter, newData));

        EditUnitTFMaterialDataDialog.registerDataCallback(this, (existingData, newData) -> ListAdapters.replaceElement(adapter, existingData, newData));
    ...
    private static final class UnitTFMaterialDataAdapter extends ListAdapter<UnitTFMaterialData, UnitTFMaterialDataAdapter.ViewHolder> {

        private static final DiffUtil.ItemCallback<UnitTFMaterialData> DATA_DIFFER = new DiffUtil.ItemCallback<>() {
            @Override
            public boolean areItemsTheSame(@NonNull UnitTFMaterialData oldItem, @NonNull UnitTFMaterialData newItem) {
                return oldItem.getUnitId() == newItem.getUnitId();
            }

            @Override
            public boolean areContentsTheSame(@NonNull UnitTFMaterialData oldItem, @NonNull UnitTFMaterialData newItem) {
                return oldItem.equals(newItem);
            }
        };

        private final BiConsumer<UnitTFMaterialData, UnitTFMaterialDataAdapter> onDeleteClick;
        private final BiConsumer<UnitTFMaterialData, UnitTFMaterialDataAdapter> onEditClick;

        public UnitTFMaterialDataAdapter(BiConsumer<UnitTFMaterialData, UnitTFMaterialDataAdapter> onDeleteClick,
                                         BiConsumer<UnitTFMaterialData, UnitTFMaterialDataAdapter> onEditClick) {
            super(DATA_DIFFER);
        ...
        public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
            ...
            if (existingLayoutManager == null) {
                ...
                layoutManager.setInitialPrefetchItemCount(item.getMaterials().size());
                ...
                llm.setInitialPrefetchItemCount(item.getMaterials().size());
            }

            final var existingAdapter = holder.binding.materialsToAdd.getAdapter();
            if (existingAdapter == null) {
                final var adapter = new TFMaterialAdapter(null, null, null);
                adapter.submitList(item.getMaterials());
                holder.binding.materialsToAdd.setAdapter(adapter);
            } else if (existingAdapter instanceof TFMaterialAdapter tfMaterialAdapter) {
                tfMaterialAdapter.submitList(item.getMaterials());
            }
        }
