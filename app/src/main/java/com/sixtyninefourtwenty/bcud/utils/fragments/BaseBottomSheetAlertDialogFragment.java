...
public abstract non-sealed class BaseBottomSheetAlertDialogFragment<V extends View> extends BottomSheetDialogFragment implements NavigationHelper, SystemServiceHelper, OnScreenMsgHelper, MiscHelper {
    ...
    protected abstract View initDialogView(V view);

    @NonNull
    @Override
    public final View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        view = initView(inflater, container);
        dialogView = initDialogView(view);
        return dialogView;
    }
    ...
    public final Toast makeToast(int textRes, ToastDuration duration) {
        return Toasts.makeToast(requireContext(), textRes, duration);
    }
    ...
    public final Snackbar makeSnackbar(int textRes, SnackbarDuration duration) {
        if (requireActivity() instanceof MainActivity ma) {
            return Snackbars.makeSnackbar(ma.getRootView(), textRes, duration);
        } else {
            assert view != null;
            return Snackbars.makeSnackbar(view, textRes, duration);
        }
    }
    ...
    public final Snackbar makeSnackbar(CharSequence text, SnackbarDuration duration) {
        if (requireActivity() instanceof MainActivity ma) {
            return Snackbars.makeSnackbar(ma.getRootView(), text, duration);
        } else {
            assert view != null;
