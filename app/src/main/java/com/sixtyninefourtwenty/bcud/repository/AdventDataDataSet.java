...
public final class AdventDataDataSet implements AdventData {
    ...
    private List<AdventStage> initStages(AssetManager assets) {
        var lines = Assets.readTextFileLines(assets, "text/advent_stages.txt", stream -> stream.map(line -> FastStringUtils.split(line, CommonConstants.PIPE)).collect(new ImmutableListCollector<>()));
    ...
    private ImmutableList<AdventBoss> initBosses(AssetManager assets) {
        ...
        var bossesFileLines = Assets.readTextFileLines(assets, "text/advent_bosses.txt", stream -> stream.map(line -> FastStringUtils.split(line, CommonConstants.PIPE)).collect(new ImmutableListCollector<>()));
        var bossInfosFileLines = Assets.readTextFileLines(assets, "text/advent_boss_infos.txt", stream -> stream.map(line -> FastStringUtils.split(line, CommonConstants.PIPE)).collect(new ImmutableListCollector<>()));
